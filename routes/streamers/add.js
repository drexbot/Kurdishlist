...
console.success('[Streamer] /servers/new.js router loaded.'.brightYellow);

app.get('/streamer/new', async (req, res) => {
...
app.post('/streamers/new', async (req, res) => {
    try {
        if (!req.user) return res.render('404.ejs', {
            bot: global.client ? global.client : null,
            path: req.path,
